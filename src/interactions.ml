...
let get_steps ?(increase_health = false) state (interaction_name : string) : step list =
  ...
  let interaction_steps : step list =
    ...
    let matches regex = Str.string_match (Str.regexp regex) interaction_name 0 in

    let get_lore prefix : string =
      let name' = Str.string_after interaction_name (String.length prefix + 1) in
      let lore =
        match List.assoc_opt name' state.global.lore with
        | None -> failwithf "lore name '%s' not found in lore.json" name'
        | Some lore -> lore
      in
      lore
    in
    ...
    match interaction_name with
    | _s when matches "^purple-pen_" ->
      remove_nail := false;
      [ STEP (PURPLE_PEN_TEXT [ "Found a purple pen with a note:"; fmt "{{purple}} %s" (get_lore "purple-pen") ]) ]
    | _s when matches "^health_" ->
      [ CURRENT_GHOST (SET_POSE READING); CURRENT_GHOST (INCREASE_HEALTH_TEXT (increase_health, get_lore "health")) ]
    | "opening-poem" ->
      [
        ...
        STEP
          (FOCUS_ABILITY_TEXT
             ( [
                 ...
                 "";
                 "";
                 "Your great strength marks you already accepted. Focus your life vapor and you shall achieve feats of \
          ...
          (TEXT
             [
               ...
               *)
               "Human Beings, these words are for you alone.";
               "";
               "Beyond this point you enter the land of Dean and Creator. Step across this threshold and obey our laws.";
             ]);
        STEP (WAIT 0.4);
    ...
    | "cutscene_fight-duncan" ->
      [
        ...
        STEP
          (DIALOGUE
             ("Duncan", "Well I'm worried about his money. My self-published novels aren't going to publish themselves."));
        STEP
          (DIALOGUE
             ( "Britta",
    ...
    | "boss-killed_LOCKER_BOY" ->
      ...
      @ [
          ...
          STEP
            (DIALOGUE
               ( "Troy",
                 "Sorry Britta, Abed knows best. But I'll always remember you as kinda slowing us down and complaining \
                  a lot." ));
          GHOST (TROY, ENTITY (SET_FACING LEFT));
