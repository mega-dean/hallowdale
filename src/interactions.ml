...
let get_steps ?(increase_health = false) state game (trigger : trigger) : step list =
  ...
  let interaction_steps : step list =
    ...
    | "boss-killed"
    | _ ->
      (* FIXME make sure this works with blocked interactions *)
      failwithf "unknown interaction prefix: %s" trigger.name_prefix
    (* FIXME can probably just match against all prefixes at the top level
      *)

    (*     match full_interaction_name with
     * | "ability:double-bouncies" ->
     ...
     *          ]);
     *   ]
     * | "cutscene:arrive-at-shirley-island" ->
     *   [
