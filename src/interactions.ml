...
let get_steps ?(increase_health = false) state game (full_interaction_name : string) interaction :
    step list =
  tmp "get steps with full_interaction_name: %s" full_interaction_name;
  ...
  let interaction_steps : step list =
    ...
    let interaction_prefix, interaction_name, trigger_kind =
      match interaction with
      | `Trigger trigger -> (trigger.name_prefix, trigger.name_suffix, Some trigger.kind)
      | `Name name -> (
        match Utils.split_at_first_opt ':' name with
        | Some prefix, name -> (prefix, name, None)
        | _ -> failwithf "name '%s' must include prefix" name)
    in
    ...
    | None -> (
      ...
      | "purple-pen" ->
        ...
        [
          STEP
            (PURPLE_PEN_TEXT
               [ "Found a purple pen with a note:"; fmt "{{purple}} %s" (get_lore ()) ]);
        ]
      | "health" ->
        [
      ...
      | _ -> (
          (* FIXME-2 should be able to match against just the name_suffix, without "info_", "ability_", etc.
        ...
        | "cutscene_fight-duncan" ->
          [
            ...
            STEP
              (DIALOGUE
                 ( "Britta",
                   "Don't regress to primal behavior just because it's allowed - we're Human \
                    Beings, not the editors of Teen Vogue." ));
            STEP (WAIT 0.7);
            STEP
              (DIALOGUE ("Britta", "They're setting a terrible example for today's young women."));
            STEP
        ...
        | "boss-killed_LOCKER_BOY" ->
          ...
          [
            ...
            STEP
              (DIALOGUE
                 ( "Troy",
                   "You used that bench to upset the balance. By the {{orange}} Vapors of \
                    Magmarath, {{white}} we will restore it." ));
          ...
          @ [
              ...
              STEP
                (DIALOGUE
                   ( "Abed",
                     "Jeff, Annie, get to {{purple}} Shirley Island! {{white}} We'll meet you \
                      there!" ));
          ...
          @ [
              ...
              STEP
                (DIALOGUE
                   ("Abed", "The third step is {{blue}} survival. {{white}} Good luck, Britta."));
              STEP (SET_FIXED_CAMERA (56, 16));
        ...
        | "ability_shade-cloak" ->
          get_ability_steps "shade_cloak" 0. 7.
            ...
            [
              ...
              "Use the cloak to scootenanny through enemies and their attacks without taking \
               damage.";
            ]
